...
background_color        = "#031400"
widget_background_color = "#031400"
text_color              = "#FFFFFF"
...
ERROR_STATUS     = "Ошибка"
STOP_STATUS      = "Остановленно"

if os_path_exists(FFMPEG_EXE_PATH): 
    print(f"[{app_name}] ffmpeg.exe нашел")
else:
    print(f"[{app_name}] ffmpeg.exe не нашел, пожалуйста, установите ffmpeg.exe")
...
class AI_upscale:
    ...
    def calculate_multiframes_supported_by_gpu(self, video_frame_path: str) -> int:
        ...
        print(f" Кадры, поддерживаемые одновременно графическим процессором: {frames_simultaneously}")
...
class MessageBox(CTkToplevel):
    ...
    def placeInfoMessageTitleSubtitle(self) -> None:
        ...
        if self._messageType == "info":
            title_subtitle_text_color = "#031400"
    ...
    try:
        while True:
            ...
            if actual_step == COMPLETED_STATUS:
                info_message.set(f"Все файлы обработаны! :)")
                stop_upscale_process()
                stop_thread()

            elif actual_step == STOP_STATUS:
                info_message.set(f"Масштабирование остановлено")
                stop_upscale_process()
                stop_thread()

            elif ERROR_STATUS in actual_step:
                info_message.set(f"Ошибка при масштабировании :(")
...
def upscale_button_command() -> None: 
    ...
    if user_input_checks():
        ...
        print("=" * 50)
        print("> Запуск увеличения разрешения:")
        print(f"  Файлы для увеличения разрешения: {len(selected_file_list)}")
        print(f"  Путь для сохранения: {(selected_output_path.get())}")
        print(f"  Выбранная AI-модель: {selected_AI_model}")
        print(f"  Выбранный GPU: {selected_gpu}")
        print(f"  Многопоточность AI: {selected_AI_multithreading}")
        print(f"  Коэффициент смешивания: {selected_blending_factor}")
        print(f"  Выбранное расширение для изображений: {selected_image_extension}")
        print(f"  Выбранное расширение для видео: {selected_video_extension}")
        print(f"  Выбранный видеокодек: {selected_video_codec}")
        print(f"  Разрешение тайлов для VRAM выбранного GPU: {tiles_resolution}x{tiles_resolution}px")
        print(f"  Коэффициент изменения размера входных данных: {int(input_resize_factor * 100)}%")
        print(f"  Коэффициент изменения размера выходных данных: {int(output_resize_factor * 100)}%")
        print(f"  Количество ядер процессора: {cpu_number}")
        print(f"  Сохранение кадров: {selected_keep_frames}")
...
def user_input_checks() -> bool:
    ...
    try: selected_file_list = file_widget.get_selected_file_list()
        ...
        info_message.set("Выберите файл")
        return False
    ...
    if selected_AI_model == MENU_LIST_SEPARATOR[0]:
        info_message.set("Выберите модель искусственного интеллекта")
        return False
    ...
    try: input_resize_factor = int(float(str(selected_input_resize_factor.get())))
        ...
        info_message.set("Входное разрешение % должно быть числом")
        return False

    if input_resize_factor > 0: input_resize_factor = input_resize_factor/100
    else:
        info_message.set("Входное разрешение % должно быть значением > 0")
        return False
    ...
    try: output_resize_factor = int(float(str(selected_output_resize_factor.get())))
        ...
        info_message.set("Выходное разрешение % должно быть числом")
        return False

    if output_resize_factor > 0: output_resize_factor = output_resize_factor/100
    else:
        info_message.set("Выходное разрешение % должно быть значением > 0")
        return False
    ...
    try: tiles_resolution = 100 * int(float(str(selected_VRAM_limiter.get())))
        ...
        info_message.set("Значением видеопамяти графического процессора должно быть число")
        return False
    ...
    if tiles_resolution > 0: 
        ...
        info_message.set("Значение видеопамяти графического процессора должно быть > 0")
        return False

    return True

def show_error_message(exception: str) -> None:
    messageBox_title    = "Ошибка"
    messageBox_subtitle = "P"
    messageBox_text     = f"\n {str(exception)} \n"
...
def open_files_action():
    ...
    print("> Загруженные файлы: " + str(uploaded_files_counter) + " => Поддерживаемые файлы: " + str(supported_files_counter))
    ...
    if supported_files_counter > 0:
        ...
        info_message.set("Готово")
    else: 
        info_message.set("Не поддерживаемые файлы :(")
...
def place_loadFile_section():
    ...
    text_drop = (" ПОДДЕРЖИВАЕМЫЕ ФАЙЛЫ \n\n "
...
def place_AI_menu():
    ...
    info_button = create_info_button(open_info_AI_model, "Модель")
...
def place_AI_blending_menu():
    ...
    info_button = create_info_button(open_info_AI_blending, "Смешивание")
...
def place_AI_multithreading_menu():
    ...
    info_button = create_info_button(open_info_AI_multithreading, "Многопоточность")
...
def place_input_output_resolution_textboxs():
    ...
    # Input resolution %
    info_button = create_info_button(open_info_input_resolution, "Входное разрешение")
    ...
    # Output resolution %
    info_button = create_info_button(open_info_output_resolution, "Выходное разрешение %")
...
def place_image_video_output_menus():
    ...
    # Image output
    info_button = create_info_button(open_info_image_output, "Вывод изображения")
    ...
    # Video output
    info_button = create_info_button(open_info_video_extension, "Вывод видео")
...
def place_video_codec_keep_frames_menus():
    ...
    # Video codec
    info_button = create_info_button(open_info_video_codec, "Кодек")
    option_menu = create_option_menu(select_video_codec_from_menu, video_codec_list, default_video_codec, width = little_menu_width)
    info_button.place(relx = column_info1,        rely = widget_row - 0.003, anchor = "center")
    option_menu.place(relx = column_1_4, rely = widget_row,         anchor = "center")

    # Keep frames
    info_button = create_info_button(open_info_keep_frames, "Сохранить кадры")
...
def place_output_path_textbox():
    ...
    background    = create_option_background()
    info_button   = create_info_button(open_info_output_path, "Вывод")
    option_menu   = create_text_box_output_path(selected_output_path) 
    active_button = create_active_button(command = open_output_path_action, text = "Выбрать", width = 60, height = 25)
...
class App():
    def __init__(self, window):
        ...
        window.iconbitmap(find_by_relative_path("Assets" + os_separator))
